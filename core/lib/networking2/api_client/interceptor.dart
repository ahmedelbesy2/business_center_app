...
class PrettyDioLogger extends Interceptor {
  ...
  void _printResponse(Response response) {
    if (response.data != null) {
      if (response.data is Map) {
        _printPrettyMap(response.data as Map, max: 20);
  ...
  void _printBlock(String msg) {
    final lines = (msg.length / maxWidth).ceil() > 500 ? 500 : (msg.length / maxWidth).ceil();
  ...
  void _printPrettyMap(
    ...
    for (var index = 0; index < data.keys.toList().asMap().length; index++) {
      ...
      if (value is Map) {
        if (compact && _canFlattenMap(value)) {
          ...
          _printPrettyMap(value, tabs: tabs, max: 10);
        }
